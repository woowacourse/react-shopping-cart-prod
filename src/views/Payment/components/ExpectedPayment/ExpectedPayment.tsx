...
function ExpectedPayment() {
  ...
  const handlePay = async () => {
    ...
    if (couponSelected) requestBody.couponId = couponSelected.id;
    ...
    <S.PayingContainer>
      <S.PayingBox>
        <S.CouponContainer>
          ...
          <CouponMessage />
          <S.CouponButton size="m" onClick={openModal} disabled={totalPrice === 0}>
            쿠폰선택
          </S.CouponButton>
        </S.CouponContainer>
        <S.PayingBackground>
        ...
        <Button size="l" primary onClick={handlePay} disabled={totalPrice === 0}>
          결제하기
        </Button>
      </S.PayingBox>
